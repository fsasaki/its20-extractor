<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	<xsl:variable name="dbpediaPrefix">http://dbpedia.org/resource/</xsl:variable>
	...
	<xsl:template match="h:p[h:a[starts-with(@href,'/wiki/') and not(contains(@href,'wiki/Template:'))]]">
		...
		<xsl:variable name="this-node">
			<xsl:apply-templates mode="stripWS"/>
		</xsl:variable>
		<xsl:variable name="offset-end" select="$offset-start + string-length($this-node)"/>
		...
		<xsl:apply-templates mode="writeTaIdentRef" select="$this-node">
